...
// 解析命令行参数
void parseArguments(int argc, char *argv[], double &exploration, double &initial_lambda, unsigned int &initial_lambda_samples)
{
    int opt;
    while ((opt = getopt(argc, argv, "e:l:s:")) != -1)
    {
        switch (opt)
        {
        case 'e': // exploration
            exploration = std::stod(optarg);
            break;
        case 'l': // initial_lambda
            initial_lambda = std::stod(optarg);
            break;
        case 's': // initial_lambda_samples
            initial_lambda_samples = std::stoi(optarg);
            break;
        default:
            std::cerr << "Usage: " << argv[0] << " -e <exploration> -l <initial_lambda> -s <initial_lambda_samples>" << std::endl;
            exit(EXIT_FAILURE);
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    parseArguments(argc, argv, exploration, initial_lambda, initial_lambda_samples);

    // 输出参数值
    std::cout << "Exploration: " << exploration << std::endl;
    std::cout << "Initial Lambda: " << initial_lambda << std::endl;
    std::cout << "Initial Lambda Samples: " << initial_lambda_samples << std::endl;
    ...
    saveVectorFieldToFile(vectorField, "/home/lhl/share/rip/scripts/landing_path_planning/VFRRT_Planner/vector_Field.txt");
    ...
    // ===== 4) 确保用于保存路径文件的目录存在 =====
    std::string pathDir = "/home/lhl/share/rip/scripts/landing_path_planning/VFRRT_Planner/path";
    ...
    // 输出参数到日志文件
    logFile << "[Info] Exploration: " << exploration << "\n";
    logFile << "[Info] Initial Lambda: " << initial_lambda << "\n";
    logFile << "[Info] Initial Lambda Samples: " << initial_lambda_samples << "\n";
