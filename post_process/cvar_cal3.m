...
% 指定方向差异文件路径
directionFilePath = 'output_data_path_e_01_lam_1001_sam_1000000/direction_differences_path_0.txt';
...
% 指定输出文件路径
outputFilePath = 'output_data_path_e_01_lam_1001_sam_1000000/direction_differences_with_cvar.txt';
...
% 循环检查每个文件是否存在
for i = 1:size(filesToCheck, 1)
    filePath = filesToCheck{i, 1};
    fileDesc = filesToCheck{i, 2};
    if ~exist(filePath, 'file')
        error('文件 "%s" 未找到。请检查路径是否正确。', filePath);
    end
end
...
% 预分配输出行的 cell 数组
outputLines = cell(directionSize(1), 1);

% ============================== 计算常数 ==============================

fprintf('计算常数 z_alpha 和 pdf_z_alpha...\n');
