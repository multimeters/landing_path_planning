% combined_plot_and_process_path.m
% 
% 功能:
% 1. 读取指定路径文件，绘制路径线，并沿路径每隔1米采样并保存采样点。
...
% 定义原始路径文件的路径
original_path_file = '\\wsl.localhost\Ubuntu/home/lhl/share/rip/scripts/landing_path_planning/VFRRT_Planner/path_e_05_lam_0011_sam_1000000/path_0.txt';

% 定义采样点输出文件的路径
sampled_path_file = '\\wsl.localhost\ubuntu\home\lhl\share\rip\scripts\landing_path_planning\post_process/sampled_path.txt';

% --------------------- 数据处理相关设置 ---------------------

% 指定输入文件名（采样后的路径）
input_filename = 'sampled_path.txt'; % 将稍后替换为采样后的路径文件
...
scale_x = 1; % x除以scale_x取整数，对应列索引
scale_y = 2; % y除以scale_y取整数，对应行索引
