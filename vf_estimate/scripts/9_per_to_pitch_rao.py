...
def process_single_file(i, per_file_template, rao_data, max_row, step, per_dir, output_dir):
    ...
    output_mat_path = os.path.join(output_dir, f'pitch_rao-{start}-{end}.mat')
    ...
    # 提取 pitch 数据
    print("提取 pitch 数据...")
    pitch_data = rao_data[row_index, 3]
    
    # 保存为新的 .mat 文件，变量名为 'pitch_rao'
    print(f"保存结果到 {output_mat_path} ...")
    sio.savemat(output_mat_path, {'pitch_rao': pitch_data})
...
def process_multiple_mat_files(per_file_template, rao_mat_path, output_dir, per_dir, num_files=16, step=256):
    """
    读取多个 per_*.mat 文件和 rao_pitch_at_0.mat 文件，处理数据并保存为 pitch_rao-*.mat 文件。
    
    参数:
    - per_file_template: str, per 文件名模板，例如 'per_{}-{}.mat'
    - rao_mat_path: str, rao_pitch_at_0.mat 文件路径
    ...
    # 加载 rao_pitch_at_0.mat 文件一次，以提高效率
    print("加载 rao_pitch_at_0.mat 文件...")
    rao_mat = sio.loadmat(rao_mat_path)
    
    # 假设变量名为 'pitch'，请根据实际情况修改
    if 'pitch' in rao_mat:
        rao_data = rao_mat['pitch']
    else:
        print("rao_pitch_at_0.mat 中的变量名有:", rao_mat.keys())
        raise KeyError("未找到变量名 'pitch' 在 rao_pitch_at_0.mat 文件中。请检查变量名。")
...
if __name__ == "__main__":
    ...
    rao_mat_path = '../data/source/rao_pitch_at_0.mat'
    output_dir = '../data/processing/output_pitch_rao'  # 输出文件夹
