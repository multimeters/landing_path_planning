...
void saveVectorFieldToFile(const VectorField &vectorField, const std::string &filename)
{
    ...
    for (double x = 0; x <= 4096; x += 1)
    {
        for (double y = 0; y <= 2048; y += 1)
        {
...
int main()
{
    ...
    start->values[0] = 40;  // Corresponding to -20 meters in index space
    start->values[1] = 40;  // Corresponding to -20 meters in index space

    ob::ScopedState<ob::RealVectorStateSpace> goal(space);
    goal->values[0] = 100;  // Corresponding to 20 meters in index space
    goal->values[1] = 100;  // Corresponding to 20 meters in index space

    // Load vector field data from files
    Eigen::MatrixXd u_subset = loadMatrixFromFile("/home/lhl/Amphi-RRT/VF_RRT/result_data/u_subset.txt", 4096, 2048);
    Eigen::MatrixXd v_subset = loadMatrixFromFile("/home/lhl/Amphi-RRT/VF_RRT/result_data/v_subset.txt", 4096, 2048);
    Eigen::MatrixXd grad_x_norm = loadMatrixFromFile("/home/lhl/Amphi-RRT/VF_RRT/result_data/grad_x_norm.txt", 4096, 2048);
    Eigen::MatrixXd grad_y_norm = loadMatrixFromFile("/home/lhl/Amphi-RRT/VF_RRT/result_data/grad_y_norm.txt", 4096, 2048);
