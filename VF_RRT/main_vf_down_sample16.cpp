...
void saveVectorFieldToFile(const VectorField &vectorField, const std::string &filename)
{
    ...
    const int numPointsX = 256;  // Number of sample points along the X-axis
    const int numPointsY = 128;  // Number of sample points along the Y-axis
    const double stepX = 4096.0 / (numPointsX - 1);  // Step size for X based on downsampled matrix dimensions
    const double stepY = 2048.0 / (numPointsY - 1);  // Step size for Y based on downsampled matrix dimensions

    auto space = std::make_shared<ob::RealVectorStateSpace>(2);
    ob::ScopedState<ob::RealVectorStateSpace> state(space);

    for (double x = 0; x <= 4096; x += stepX)
    {
        for (double y = 0; y <= 2048; y += stepY)
        {
...
int main()
{
    ...
    Eigen::MatrixXd u_combined = loadMatrixFromFile("/home/lhl/Amphi-RRT/VF_RRT/build/combined_u_vector_field_downsampled.txt", 256, 128);
    Eigen::MatrixXd v_combined = loadMatrixFromFile("/home/lhl/Amphi-RRT/VF_RRT/build/combined_v_vector_field_downsampled.txt", 256, 128);
    ...
    VectorField vectorField = [&u_combined, &v_combined](const ob::State *state) -> Eigen::VectorXd {
        ...
        double x = realState->values[0] / (0.5 * 16);  // Convert meters to downsampled matrix index
        double y = realState->values[1] / (0.5 * 16);  // Convert meters to downsampled matrix index
        ...
        i = std::min(std::max(i, 0), 255);
        j = std::min(std::max(j, 0), 127);
    ...
    double exploration = 1.5;  // Example value
    double initial_lambda = 5.5;  // Example value
    unsigned int initial_lambda_samples = 1000000;  // Example value
