...
% 对eta进行测试
error_count_eta = test_variable('eta', original_dir, mat_dir_eta, sample_files, sample_rows, sample_cols, num_samples, rows, cols);

% 对u进行测试
error_count_u = test_variable('u', original_dir, mat_dir_u, sample_files, sample_rows, sample_cols, num_samples, rows, cols);

% 对v进行测试
error_count_v = test_variable('v', original_dir, mat_dir_v, sample_files, sample_rows, sample_cols, num_samples, rows, cols);
...
% 定义函数来测试不同的变量
function error_count = test_variable(var_name, original_dir, mat_dir, sample_files, sample_rows, sample_cols, num_samples, rows, cols)
    error_count = 0;
    for i = 1:num_samples
